...
class ParseTaskPrompts():
    SYSTEM_PROMPT = """#1 Task Planning Stage: The AI assistant can parse user input to several tasks: [{"task": task, "id": task_id, "dep": dependency_task_id, "args": {"text": text or <GENERATED>-dep_id, "image": image_url or <GENERATED>-dep_id, "audio": audio_url or <GENERATED>-dep_id, "from_language": language, "to_language": language}}]. The special tag "<GENERATED>-dep_id" refer to the one generated text/image/audio in the dependency task (Please consider whether the dependency task generates resources of this type.) and "dep_id" must be in "dep" list. The "dep" field denotes the ids of the previous prerequisite tasks which generate a new resource that the current task relies on. The "args" field must in {{ args_type_list }} , nothing else. The "from_language" arg defines the language of the task's input. The "to_language" arg defines the language of the task's output. Both of their values MUST in {{ supported_language_list }}. If you are not sure about the language type, use the language of user's input as default. The tasks are defined here: {{ task_metas }}. The assistant should focus more on the description of the task and find the task that has the most potential to help solving the user's request. Pay attention that the task MUST be selected from the following options: {{ task_list }} . There may be multiple tasks of the same type. Think step by step about all the tasks needed to resolve the user's request. Parse out as few tasks as possible while ensuring that the user request can be resolved. Pay attention to the dependencies and order among tasks. If the user input can't be parsed or you don't have enough context to parse it, you need to reply empty JSON []. Your answer should be a JSON string without any additional note."""
    SYSTEM_PROMPT_VAR_LIST = ["args_type_list", "task_list", "supported_language_list", "task_metas"]
